...
const SERVICE_STAT_BGS: Record<DashboardService, string[]> = {
  FLT: [
...
export class Dashboard implements OnInit {
  ...
  protected readonly statCards = computed<StatCard[]>(() => {
    ...
    const bgs = SERVICE_STAT_BGS[service];

    return [
      { label: `${service} – Total`, value: stats.total, icon: 'monitoring', cardBg: bgs[0], loading: this.loading() },
      { label: `${service} – Pending`, value: stats.pending, icon: 'pending_actions', cardBg: bgs[1], loading: this.loading() },
      { label: `${service} – Approved`, value: stats.approved, icon: 'check_circle', cardBg: bgs[2], loading: this.loading() },
      { label: `${service} – Rejected/Cancelled`, value: stats.rejected, icon: 'cancel', cardBg: bgs[3], loading: this.loading() },
    ];
  });
