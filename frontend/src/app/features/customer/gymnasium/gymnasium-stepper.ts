  ...
  template: `
        ...
        <div class="flex flex-col gap-4 animate-fade-in">
          <div class="flex items-start justify-between gap-3">
            <div>
              ...
              <p class="text-sm text-gray-500 mt-0.5">Review your date and time selections.</p>
            </div>
            <button type="button" (click)="addMoreDates.emit()"
              ...
              @for (slot of dateSlots(); track slot.date) {
                <div class="rounded-xl border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-4 shadow-sm">
                  <div class="flex items-center justify-between mb-3">
                    ...
                    <button type="button" (click)="removeDateSlot(slot.date)"
          ...
          <div [formGroup]="detailsForm" class="flex flex-col gap-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div class="sm:col-span-2 flex flex-col gap-2">
                ...
                <input uiInput id="eventTitle" formControlName="eventTitle" placeholder="e.g. Annual Sports Festival" />
              ...
              <div class="sm:col-span-2 flex flex-col gap-2">
                ...
                <input uiInput type="number" id="numberOfAttendees" formControlName="numberOfAttendees" placeholder="e.g. 100" min="1" />
                @if (detailsForm.get('numberOfAttendees')?.invalid && detailsForm.get('numberOfAttendees')?.touched) {
                  @if (detailsForm.get('numberOfAttendees')?.errors?.['required']) {
                    <p class="text-xs text-red-500">Number of attendees is required.</p>
                  } @else if (detailsForm.get('numberOfAttendees')?.errors?.['min']) {
              ...
              <div class="sm:col-span-2 flex flex-col gap-2">
                ...
                <textarea
                  ...
                  rows="3"
                  placeholder="Any special requests, setup requirements, or notes for the Gymnasium team..."
            ...
            <div class="flex flex-col gap-3">
              ...
              @if (equipmentLoading) {
                ...
                <div class="py-4 text-sm text-gray-400 italic">No equipment available for Gymnasium.</div>
              } @else {
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  @for (eq of availableEquipment; track eq.id) {
                    <button type="button" (click)="toggleEquipment(eq)"
        ...
        <div class="flex flex-col gap-4 animate-fade-in">
          ...
          <div class="mt-2 rounded-xl bg-gray-50 dark:bg-zinc-800/60 border border-gray-200 dark:border-zinc-700 p-4 flex flex-col gap-3">
            ...
            <div class="flex flex-col gap-1.5 text-sm text-gray-600 dark:text-zinc-400">
              ...
              <div class="flex gap-2"><span class="font-medium text-gray-800 dark:text-zinc-200 min-w-24">Attendees:</span> {{ detailsForm.get('numberOfAttendees')?.value || '—' }}</div>
      ...
      @if (submitted()) {
        <div class="flex flex-col items-center justify-center gap-4 py-12 text-center animate-fade-in">
          ...
          <div>
            ...
            <p class="text-sm text-gray-500 mt-1 max-w-xs">Your Gymnasium reservation request has been sent. You will be notified once it's confirmed.</p>
          </div>
    ...
    @if (!submitted()) {
      <div class="shrink-0 p-6 flex gap-3 sticky bottom-0 z-20 bg-white/40 dark:bg-zinc-900/40 backdrop-blur-md border-t border-gray-100 dark:border-zinc-800">
        ...
        @if (currentStep() < 3) {
          <button uiButton variant="primary" type="button" class="flex-1" [disabled]="!canProceed()" (click)="nextStep()">
            NEXT
          </button>
        } @else {
          <button uiButton variant="primary" type="button" class="flex-1" [disabled]="!canSubmit() || submitting()" (click)="submit()">
            @if (submitting()) {
...
export class GymnasiumStepper implements OnChanges {
  @Input() selectedDates: ReservedDateSlot[] = [];
  @Input() availableEquipment: GymEquipmentItem[] = [];
  ...
  readonly selectedEquipment = signal<GymEquipmentItem[]>([]);

  readonly detailsForm = new FormGroup({
    eventTitle: new FormControl('', Validators.required),
    department: new FormControl('', Validators.required),
    organization: new FormControl('', Validators.required),
    numberOfAttendees: new FormControl<number | null>(null, [Validators.required, Validators.min(1)]),
  ...
  toggleEquipment(eq: GymEquipmentItem): void {
  ...
  formatDateDisplay(dateStr: string): string {
    ...
    return new Date(year, month - 1, day).toLocaleDateString('en-US', { weekday: 'short', month: 'long', day: 'numeric', year: 'numeric' });
  }

  canProceed(): boolean {
    if (this.currentStep() === 1) return this.dateSlots().length > 0;
    if (this.currentStep() === 2) return this.detailsForm.valid;
    return false;
  }
  ...
  submit(): void {
    ...
    const payload: GymReservationPayload = {
